  ...
  return async (request: Request, actor: Actor | null, requestId: string): Promise<Response> => {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        } else if (method === "POST" && id && parts.length === 5 && action === "publish" && (resource === "modules" || resource === "lessons" || resource === "activities")) {
          result = await publishContent(db, actor, courseId, resource, id, body, requestId);
        } else if (method === "POST" && id && parts.length === 5 && action === "archive" && (resource === "modules" || resource === "lessons" || resource === "materials" || resource === "activities")) {
          result = await archiveContent(db, actor, courseId, resource, id, body, requestId);
        } else if (method === "POST" && id && parts.length === 5 && resource === "lessons" && action === "complete") {
        ...
        } else if (method === "POST" && id && parts.length === 5 && resource === "submissions" && action === "grade") {
