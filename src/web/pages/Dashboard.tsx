...
interface DashboardData { academic: string | null; courses: number; classes: number; tasks: never[]; events: never[] }
export function Dashboard({ actor, timezone, onExpired }: { actor: Actor; timezone: string; onExpired: () => void }) {
  ...
  return <>
    ...
    {error ? <ErrorState message={error} retry={() => setAttempt(value => value + 1)} /> : !data ? <LoadingState /> : <>
      ...
      <div className="dashboard-grid"><Card><div className="card-heading"><h2>Tugas hari ini</h2><span className="badge">Today's Tasks</span></div><EmptyState symbol="✓" title="Belum ada tugas untuk ditampilkan" description="Tugas dan aktivitas belajar akan tampil di sini saat modul pembelajaran tersedia." /></Card><Card><div className="card-heading"><h2>Agenda mendatang</h2><span className="calendar-icon" aria-hidden="true">▦</span></div><EmptyState title="Belum ada agenda" description="Jadwal akademik dan kegiatan sekolah akan hadir di ruang ini." /></Card></div>
