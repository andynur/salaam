...
function CourseSessions({ courseId, timezone, onExpired }: { courseId: string; timezone: string; onExpired: () => void }) {
  ...
  return <div className="lesson-workspace"><div className="filter-bar"><a href="/attendance">Kehadiran</a></div><PageHeader title={data?.course.name ?? "Sesi kelas"} description="Kehadiran selalu dicatat pada sesi pertemuan." />
    ...
    {tab === "report" ? <Report courseId={courseId} onExpired={onExpired} /> : <>
      {data?.course.canManage && <div className="filter-bar"><Button aria-expanded={creating} aria-controls="create-session" onClick={() => { setCreating(!creating); setRequestKey(crypto.randomUUID()); }}>Buat sesi</Button></div>}
      {creating && <Card className="lesson-card"><div id="create-session"><h2>Sesi baru</h2><MutationForm path={`${root}/${courseId}/sessions`} label="Simpan sesi" onExpired={onExpired} saved={() => { setCreating(false); setRevision(v => v + 1); }} body={form => ({ title: form.get("title"), startsAt: fromLocalInput(form.get("startsAt")), endsAt: fromLocalInput(form.get("endsAt")), note: form.get("note"), requestKey })}>
      ...
      </MutationForm><p className="card-hint">Daftar santri aktif disimpan saat sesi dibuat. Durasi maksimal 24 jam.</p></div></Card>}
