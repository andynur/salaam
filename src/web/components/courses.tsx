  ...
  return <>
    ...
    <Card><div className="card-heading catalog-heading"><h2>Course Anda</h2><div className="card-tools"><Search change={value => { setQ(value); setOffset(0); }} />
      ...
      {error ? <ErrorState message={error} retry={retry} /> : !data ? <LoadingState /> : !visibleCourses.length ? <EmptyState icon={emptyIcon} title="Tidak ada course" description={q || filtered ? "Coba ubah pencarian atau filter Anda." : emptyDescription} action={filtered ? <Button className="button-secondary button-small" onClick={resetFilters}>Atur ulang filter</Button> : undefined} />
        : <div className="catalog-groups">{groups.map(group => <section className="catalog-group" key={group.value}><div className="catalog-group-heading"><h3>Kelas {group.value}</h3><span className="catalog-group-count">{group.courses.length} course</span></div>
          <div className="course-grid">{group.courses.map(course => <a className="course-tile catalog-tile" href={href(course)} key={course.id}><div className="learning-row"><span className={`course-avatar course-avatar-${gradeKey(course.className)}`} aria-hidden="true">{initials(course.name)}</span>{badge ? badge(course) : <Status published={course.published} />}</div><h3>{course.name}</h3><p><span className="grade-chip tone-soft-neutral">{course.className}</span> {course.term} · {course.year}</p><span className="course-open">{openLabel(course)}<Icon name="arrowRight" /></span></a>)}</div></section>)}</div>}
