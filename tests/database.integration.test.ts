...
describe.skipIf(!url)("PostgreSQL integration (isolated temporary schema)", () => {
  ...
  test("rollback undoes exactly the latest migration; reset replays every migration from empty", async () => {
    expect(await rollback(db)).toBe("0039_academic_calendar_reference.sql");
    expect(await rollback(db)).toBe("0038_editable_academic_calendar.sql");
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'academic_calendar_events' AND column_name IN ('version', 'updated_at', 'request_key', 'last_operation')`).length).toBe(0);
    expect(await rollback(db)).toBe("0037_course_certificates.sql");
    expect((await db`SELECT to_regclass('course_certificates') AS relation`)[0].relation).toBeNull();
    ...
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'classroom_roster' AND column_name = 'removed_at'`).length).toBe(0);
    expect(await rollback(db)).toBe("0023_project_file_deliverables.sql");
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'projects' AND column_name = 'deliverable_file_id'`).length).toBe(0);
    expect(await rollback(db)).toBe("0022_project_task_comments.sql");
    expect((await db`SELECT to_regclass('project_task_comments') AS relation`)[0].relation).toBeNull();
    expect(await rollback(db)).toBe("0021_project_task_metadata.sql");
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'project_tasks' AND column_name IN ('due_at', 'labels')`).length).toBe(0);
    expect(await rollback(db)).toBe("0020_assessment_question_pools.sql");
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'assessment_settings' AND column_name = 'selection_count'`).length).toBe(0);
    expect(await rollback(db)).toBe("0019_question_imports.sql");
    expect((await db`SELECT to_regclass('question_imports') AS relation`)[0].relation).toBeNull();
    expect(await rollback(db)).toBe("0018_assessment_accommodations.sql");
    expect((await db`SELECT to_regclass('assessment_accommodations') AS relation`)[0].relation).toBeNull();
    expect(await rollback(db)).toBe("0017_assessment_marking.sql");
    expect((await db`SELECT column_name FROM information_schema.columns WHERE table_schema = ${schema} AND table_name = 'assessment_settings' AND column_name = 'scoring_mode'`).length).toBe(0);
    expect(await rollback(db)).toBe("0016_survey_questionnaires.sql");
    expect((await db`SELECT to_regclass('survey_responses') AS relation`)[0].relation).toBeNull();
    expect((await db`SELECT to_regclass('survey_questions') AS relation`)[0].relation).toBeNull();
    ...
    expect(result.rolledBack).toEqual(allNames.slice(0, -30).reverse());
