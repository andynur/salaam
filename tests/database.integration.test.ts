...
describe.skipIf(!url)("PostgreSQL integration (isolated temporary schema)", () => {
  ...
  test("rollback undoes exactly the latest migration; reset replays every migration from empty", async () => {
    expect(await rollback(db)).toBe("0005_project_learning.sql");
    expect((await db`SELECT to_regclass('projects') AS relation`)[0].relation).toBeNull();
    expect((await db`SELECT to_regclass('questions') AS relation`)[0].relation).not.toBeNull();
    expect((await db`SELECT name FROM schema_migrations ORDER BY name`).map((row: { name: string }) => row.name))
      .toEqual(["0001_identity.sql", "0002_academic_foundation.sql", "0003_learning_core.sql", "0004_assessment_engine.sql"]);

    const allNames = (await readMigrations("database/migrations")).map(migration => migration.name);
    const result = await reset(db);
    expect(result.rolledBack).toEqual(["0004_assessment_engine.sql", "0003_learning_core.sql", "0002_academic_foundation.sql", "0001_identity.sql"]);
